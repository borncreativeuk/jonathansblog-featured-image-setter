...
{
	add_options_page("Set Featured Images", "Set Featured Images", "edit_others_posts", "Set Featured Images", "jonathansblog_featured_image_setter_admin_view");
}
...
function jonathansblog_featured_image_setter_admin_view()
{
	...
	if (file_exists(plugin_dir_path(__FILE__) . '../views/jonathansblog-featured-image-setter-admin-view.php')) {
		include_once plugin_dir_path(__FILE__) . '../views/jonathansblog-featured-image-setter-admin-view.php';
	}
}
...
function jonathansblog_featured_image_setter_admin_save_stuff()
{
	...
	if (!empty($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'], 'jonathansblog-featured-image-setter-form-nonce')) {
		...
		wp_die(
			__('Invalid nonce specified', 'jonathansblog-featured-image-setter'),
			__('Error', 'jonathansblog-featured-image-setter'),
			array(
				'response'     => 403,
				'back_link' => $url,
			)
